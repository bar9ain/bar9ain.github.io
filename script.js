...
setTimeout(() => {
  ...
  const year = document
    .querySelector(".main-content .mb-3.text-white")
    .innerText.split("/")
    .pop()
    .trim();
  ...
  window.titles = Array.from(document.querySelectorAll(".row.grid .card")).map(
    (x) => ({
      title: x.querySelector("a[href*='/lich-chieu/'], a[href*='/phim/']")
        ?.title,
      release_date: formatDate(x.querySelector(".text-muted").innerText.trim()),
      image: x.querySelector("img").src,
      moveek_url: x
        .querySelector("a[href*='/lich-chieu/'], a[href*='/phim/']")
        .href.replace("lich-chieu", "phim"),
      romanized: jsrmvi.removeVI(
        x.querySelector("a[href*='/lich-chieu/'], a[href*='/phim/']")?.title,
        { concatBy: " " }
      ),
    })
  );
